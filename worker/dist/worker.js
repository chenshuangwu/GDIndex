...
(function () {
  ...
  class GoogleDrive {
    ...
    async listFolder(id, pageToken, originKeyword) {
      ...
      const getList = pageToken => {
        const qs = {
          ...
          fields: 'files(id,name,mimeType,size,createdTime,modifiedTime,iconLink,thumbnailLink),nextPageToken',
      ...
      const resp = await getList(pageToken); // files.push(...resp.files)
    ...
    */
    ...
    async findParentFilesRecursion(child_id, rootId, contain_myself = true) {
      const gd = this; // const gd_root_id = gd.root.id;
  ...
  async function onGet(request) {
    ...
    if (path.startsWith('/~_~_gdindex/resources/')) {
      ...
      const r = await fetch(`https://raw.githubusercontent.com/chenshuangwu/GDIndex/master/web/dist/${remain}`);
      ...
      const r = await fetch(`https://lh3.googleusercontent.com/${remain}`);
      return new Response(r.body, {
        Headers: {
          'Content-Type': mime.getType(remain) + '; charset=utf-8',
          'Cache-Control': 'max-age=600'
      ...
      const r = await fetch(`https://drive-thirdparty.googleusercontent.com/${remain}`);
      return new Response(r.body, {
        Headers: {
          'Content-Type': mime.getType(remain) + '; charset=utf-8',
          'Cache-Control': 'max-age=600'
  ...
  async function onPost(request) {
    ...
    if (path === '/id2path') {
      ...
      return new Response(JSON.stringify(await gd.listFolderByPath(path, rootId, pageToken, keyword)), {
        headers: {
          'Content-Type': 'application/json'
        }
      });
